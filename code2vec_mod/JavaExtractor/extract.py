...
def ExtractFeaturesForDir(args, tmpdir, dir_, prefix):
    ...
    # dir_.split('/')[-1]  worsk only on Linux and it was replaced with platform independent basename
    # outputFileName = tmpdir + prefix + dir_.split('/')[-1]
    outputFileName = os.path.join(tmpdir, f"{prefix}{os.path.basename(dir_)}")
    failed = False
