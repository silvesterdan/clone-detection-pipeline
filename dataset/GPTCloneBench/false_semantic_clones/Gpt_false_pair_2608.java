public static String translateToEnglish (String phrase) {
 ...
 public static String switchingToEnglish (String phrase) {
    if (phrase == null) {
        return null;
    }
    String change = phrase.replace ('4', 'a').replace ('1', 'l').replace ('2', 'z').replace ('5', 's').replace ('8', 'b').replace('0', 'o').replace('7', 't').replace("|_|", "u").replace("3", "e");
    change = change.replace(" e ", " 3 ");
    if (change.startsWith("e ")) {
        change = "3 " + change.substring(2);
    }
    if (change.endsWith(" e")) {
        change = change.substring(0, change.length() - 2) + " 3";
    }
    return change;
}
