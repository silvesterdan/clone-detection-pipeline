public int [] solution (String S, int [] P, int [] Q) {
 ...
 public int [] solution (String S, int [] P, int [] Q) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        lastOccurrences[i] = (lastA << 16) | (lastC << 8) | lastG;
    }
    for (int i = 0; i < M; i++) {
        int startIndex = P[i];
        int endIndex = Q[i];
        int minimum = 4;
        for (int n = 0; n < 3; n++) {
            int lastOccurence = getLastNucleotideOccurrence(startIndex, endIndex, n, lastOccurrences);
            if (lastOccurence > 0) {
                minimum = n + 1;
                break;
            }
