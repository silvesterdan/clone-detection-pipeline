public Serializable generate (SessionImplementor session, Object object) throws HibernateException {
 ...
 public Serializable generate(SessionImplementor session, Object object) throws HibernateException {
    ...
    try {
        ...
        statement = connection.createStatement();
        try {
            resultSet = statement.executeQuery("SELECT nextid FROM my_seq");
        }
        catch (Exception e) {
            System.out.println ("In catch, cause : Sequence is not available.");
            statement.execute ("CREATE table my_seq (nextid INT NOT NULL)");
            statement.executeUpdate ("INSERT INTO my_seq VALUES(0)");
            statement.executeUpdate ("UPDATE my_seq SET nextid=last_insert_id(nextid+1)");
            resultSet = statement.executeQuery ("SELECT nextid FROM my_seq");
        }
        if (resultSet.next()) {
            int nextValue = resultSet.getInt (1);
