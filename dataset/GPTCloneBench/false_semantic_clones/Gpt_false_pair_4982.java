...
public static void printTable (String [] countries, int [] populations) {
    ...
    int maxCountryLength = Arrays.stream (countries).map (String :: toString).mapToInt (String :: length).max ().getAsInt ();
    int maxPopLength = Arrays.stream (populations).mapToObj (Integer :: toString).mapToInt (String :: length).max ().getAsInt ();
    for (int i = 0; i < countries.length; i++)
    {
        System.out.printf(String.format("%%-%ds | %%%dd%%n", maxCountryLength, maxPopLength), countries[i],
          populations[i]);
    }
}
