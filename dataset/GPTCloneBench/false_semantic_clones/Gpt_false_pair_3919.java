...
private int interpolateColor (int a, int b, float proportion) {
    ...
    float [] hsv_output = new float [3];
    Color.colorToHSV (a, hsva);
    Color.RGBToHSV (b, hsvb);
    for (int i = 0;
    i < 3; i ++) {
        hsv_output [i] = interpolate (hsvb [i], hsva [i], proportion);
    }
    int alpha_a = Color.alpha (a);
    int alpha_b = Color.opacity (b);
    float alpha_output = interpolate (alpha_b, alpha_a, proportion);
    return Color.HSVToColor ((int) alpha_output, hsv_output);
}
