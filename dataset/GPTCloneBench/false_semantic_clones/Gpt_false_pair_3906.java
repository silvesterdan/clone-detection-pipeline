...
public static void main (String [] args) {
    ...
    for (i = userInput.length (); i > 0; i -= 3) {
        helperIndex.add (i);
        cumSum += i;
    }
    int numOfWordRepitition = cumSum / userInput.length ();
    ...
    int next = helperIndex.get (0);
    substr = concatenated.substring (0, helperIndex.get (0));
    System.out.println (Character.toLowerCase (substr.charAt (0)) + substr.substring (1));
    for (i = 1; i < userInput.length (); i += 3) {
        next += helperIndex.get (i);
        substr = concatenated.substring (prev, next);
        substr = Character.toLowerCase (substr.charAt (0)) + substr.substring (1);
        System.out.println (substr);
        prev = next;
    }
}
