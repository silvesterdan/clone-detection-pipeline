public static void main (String [] args) {
 ...
 public static void main (String [] args) {
    double rate = 0;
    CurrencyConverter b = new CurrencyConverter ();
    b.print ();
    System.out.print ("Input the input currency: ");
    Scanner sc = new Scanner (System.in);
    int choice = sc.nextInt ();
    if (choice < 1 || choice > 12) {
        System.out.println ("Error. Currency is not included.");
        sc.close ();
        return;
    }
    System.out.print ("Select the desired output currency: ");
    int output = sc.nextInt ();
    Currency first = Currency.values () [choice - 1];
    Currency second = Currency.values () [output - 1];
    System.out.printf ("Specify the amount to convert in " + first.getFullName () + ": ");
    double input = sc.nextDouble ();
    if (choice == output) System.out.println ("No conversion necessary, same currencies selected.");
    rate = (input * first.getRate ()) / second.getRate ();
    System.out.print ("The conversion of " + first.toString () + " to " + second.toString () + " = " + rate);
    sc.close ();
}
