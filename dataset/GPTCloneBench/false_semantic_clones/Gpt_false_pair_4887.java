...
public String [] [] allUniqueCombos () {
    int n = dataStructure.keySet ().size ();
    int solns = 1;
    for (Vector < String > vector : dataStructure.values ()) {
        solns *= vector.size ();
    }
    String [] [] allCombos = new String [solns + 1] [];
    allCombos [0] = dataStructure.keySet ().toArray (new String [n]);
    for (int i = 0;
    i < solns; i ++) {
        Vector < String > combo = new Vector < String > (n);
        int j = 1;
        for (Vector < String > vec : dataStructure.values ()) {
            combo.add (vec.get ((i / j) % vec.size ()));
            j *= vec.size ();
        }
        allCombos [i + 1] = combo.toArray (new String [n]);
    }
    return allCombos;
}
