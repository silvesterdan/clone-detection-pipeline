public void draw (Graphics2D g) {
 ...
 public void draw (Graphics2D g) { float arrowRatio = 0.8f;
    float arrowLength = 95.0f;
    BasicStroke stroke = (BasicStroke) g.getStroke ();
    float endX = 450.0f;
    float veeX = endX - stroke.getLineWidth () * 0.5f / arrowRatio;
    Path2D.Float path = new Path2D.Float ();
    ...
    path.quadTo (waistX, - waistY, endX, 0.0f);
    path.quadTo (waistX, waistY, veeX - arrowLength, arrowWidth);
    path.lineTo (veeX - arrowLength * 0.4f, 0.0f);
    path.lineTo (veeX - arrowLength, - arrowWidth);
    g.setColor (Color.GREEN);
    g.fill (path);
    g.setColor (Color.BLUE);
    g.draw (new Line2D.Float (60.0f, 0.0f, veeX - arrowLength * 0.7f, 0.0f));
}
