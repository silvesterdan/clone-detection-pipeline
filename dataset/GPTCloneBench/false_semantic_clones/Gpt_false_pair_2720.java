	...
	public void invoke(Request request, Response response) throws IOException, ServletException {
        ...
        if (session != null) {
            ...
            if (session instanceof ReplicatedSession) {
                ((ReplicatedSession) session).setIsDirty(true);
                if (log.isDebugEnabled()) {
                    log.debug(getInfo() + ": [session=" + session.getId() + ", context=" + context + ", request=" + task + "] set DIRTY");
                }
            } else {
                ...
                if (log.isDebugEnabled()) {
                    log.debug(getInfo() + ": [session=" + session.getId() + ", context=" + context + ", request=" + task + "] cycle atrributes=" + cycledNames + "");
                }
            }
        } else {
            String id = request.getRequestedSessionId();
