...
public static void displayTable(String[] countries, int[] populations) {
    if (countries.length != 0) {
        int maxNameLenInput = countries[0].length();
        int maxPopLenInput = String.valueOf(populations[0]).length();
        for (int i = 0; i < countries.length; i++) {
            int countLength = countries[i].length();
            int popLength = String.valueOf(populations[i]).length();
            if (countLength > maxNameLenInput)
                maxNameLenInput = countLength;
            if (popLength > maxPopLenInput)
                maxPopLenInput = popLength;
        }
        for (int i = 0; i < countries.length; i++) {
            System.out.print(countries[i]);
            for (int j = 0; j < (maxNameLenInput - countries[i].length()); j++)
                System.out.print(" ");
            System.out.print(" | ");
            for (int k = 0; k < (maxPopLenInput - String.valueOf(populations[i]).length()); k++)
                System.out.print(" ");
            System.out.println(populations[i]);
        }
    }
