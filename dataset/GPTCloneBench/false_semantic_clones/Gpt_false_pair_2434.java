...
public int[] solution(String S, int[] P, int[] Q) {
    int[] result = new int[P.length];
    Map<Character, Integer> factorsMap = new HashMap<Character, Integer>();
    factorsMap.put('A', 1);
    factorsMap.put('C', 2);
    factorsMap.put('G', 3);
    factorsMap.put('T', 4);
    ...
    for (int i = 0; i < P.length; i++) {
        ...
        if (start == 0) {
            if (factor1[end] > 0) {
                result[i] = factorsMap.get('A');
            } else if (factor2[end] > 0) {
                result[i] = factorsMap.get('C');
            } else if (factor3[end] > 0) {
                result[i] = factorsMap.get('G');
            } else {
                result[i] = factorsMap.get('T');
            }
        } else {
            if (factor1[end] > factor1[start - 1]) {
                result[i] = factorsMap.get('A');
