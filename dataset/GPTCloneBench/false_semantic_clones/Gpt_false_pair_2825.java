public static void main (String [] args) {
  ...
  public static void main (String [] args) {
    ...
    int choice = sc.nextInt ();
    if (choice < 1 || choice > 12) {
        System.out.println ("Error. Input currency not valid.");
        sc.close ();
        return;
    }
    ...
    Currency first = Currency.values () [choice - 1];
    Currency second = Currency.values () [output - 1];
    System.out.printf ("Input the desired amount in " + first.getFullName () + ": ");
    double input = sc.nextDouble ();
    if (choice == output) System.out.println ("Unnecessary conversion, same currencies.");
    rate = (input * first.getRate ()) / second.getRate ();
    System.out.print ("The result of " + first.toString () + " to " + second.toString () + " = " + rate);
    sc.close ();
}
