...
public static void main (String [] args) {
    ...
    CurrencyConverter w = new CurrencyConverter ();
    w.print ();
    System.out.print ("Selecti wan patrakel: ");
    Scanner sc = new Scanner (System.in);
    int choice = sc.nextInt ();
    if (choice < 1 || choice > 12) {
        System.out.println ("404 nou faind XD");
        sc.close ();
        return;
    }
    System.out.print ("Selecti wan patrakel to betika: ");
    int output = sc.nextInt ();
    Currency first = Currency.values () [choice - 1];
    Currency second = Currency.values () [output - 1];
    System.out.printf ("Nou enta input in " + first.getFullName () + ": ");
    double input = sc.nextDouble ();
    if (choice == output) System.out.println ("Troller ._. pabobo");
    rate = (input * first.getRate ()) / second.getRate ();
    System.out.print ("Betika de " + first.toString () + " to " + second.toString () + " = " + rate);
    sc.close ();
}
