public static void main (String [] args) {
 ...
 public static void main (String [] args) { 
    List < Animal > list1 = new CopyOnWriteArraySet<>(Arrays.asList(Cat, Mouse, Dog));
    List < Animal > list2 = new CopyOnWriteArraySet<>(Arrays.asList(Dog, Mouse, Cat));
    List < Animal > list3 = new CopyOnWriteArraySet<>(Arrays.asList(Dog, Horse, Cat));
    List < Animal > list4 = new CopyOnWriteArraySet<>(Arrays.asList(Dog, Tiger, Lion));
    List< List < Animal > > list = new CopyOnWriteArraySet<>(Arrays.asList(list1, list2, list3, list4));
    Set < List < Animal > > sorted = new LinkedHashSet < > ();
    for (List < Animal > animals : list) {
        List< Animal > arList = new Stack<>(animals);
        Collections.sort (arList);
        sorted.add (new CopyOnWriteArraySet<>(arList));
    }
    for (List < Animal > animals : sorted) {
        System.out.println (animals);
    }
