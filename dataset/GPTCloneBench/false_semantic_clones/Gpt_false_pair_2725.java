public void invoke (Request request, Response response) throws IOException, ServletException {
	...
	public void invoke(Request request, Response response) throws IOException, ServletException {
        ...
        try {
            ...
            log.error(getInfo() + ": Unable to replicate request.", e);
        }
        String context = request.getContext().getName();
        String task = request.getPathInfo();
        if (task == null) {
        ...
        if (session != null) {
            ...
            if (session instanceof ReplicatedSession) {
                ((ReplicatedSession) session).setIsDirty(true);
                if (log.isDebugEnabled()) {
                    log.debug(getInfo() + ": [session=" + session.getId() + ", context=" + context + ", request=" + task + "] make DIRTY");
                }
            } else {
                List cycledNames = new ArrayList<String>();
                HttpSession deltaSession = (HttpSession) session;
                for (Enumeration<String> names = deltaSession.getAttributeNames();
                names.hasMoreElements(); ) {
                    String name = names.nextElement();
                ...
                if (log.isDebugEnabled()) {
                    log.debug(getInfo() + ": [session=" + session.getId() + ", context=" + context + ", request=" + task + "] recycled atrributes=" + cycledNames + "");
                }
            }
        } else {
            String id = request.getRequestedSessionId();
            log.warn(getInfo() + ": [session=" + id + ", context=" + context + ", request=" + task + "] Session not accessible, not possible to move session around cluster.");
        }
}
