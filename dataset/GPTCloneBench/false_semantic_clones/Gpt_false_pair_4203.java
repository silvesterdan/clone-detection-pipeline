...
public static void printTable(String[] countries, int[] populations) {
    if (countries.length != 0) {
        int maxNameLength = countries[0].length();
        int maxPopLength = String.valueOf(populations[0]).length();
        for (int i = 0; i < countries.length; i++) {
            int countLength = countries[i].length();
            int popLength = String.valueOf(populations[i]).length();
            if (countLength > maxNameLength)
                maxNameLength = countLength;
            if (popLength > maxPopLength)
                maxPopLength = popLength;
        }
        for (int i = 0; i < countries.length; i++) {
            System.out.print(countries[i]);
            for (int j = 0; j < (maxNameLength - countries[i].length()); j++)
                System.out.print(" ");
            System.out.print(" | ");
            for (int k = 0; k < (maxPopLength - String.valueOf(populations[i]).length()); k++)
                System.out.print(" ");
            System.out.println(populations[i]);
        }
    }
