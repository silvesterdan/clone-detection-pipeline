public static void main (String [] args) {
 ...
 public static void main (String [] args) {
    Scanner scan = new Scanner (System.in);
    int numOfObjects = 0;
    System.out.print ("List The Quantity of Items: ");
    try {
        numOfObjects = Integer.parseInt (scan.nextLine ().trim ());
    } catch (NumberFormatException e) {
        System.out.print ("Number of items you entered is invalid!");
        System.exit (0);
    }
    Grocery grocery = new Grocery (numOfObjects);
    for (int i = 0;
    i < numOfObjects; i ++) {
        System.out.print ("Name Item " + (i + 1) + " : ");
        if (! grocery.addItem (scan.nextLine ())) {
            System.out.println ("Reduplication Of Item Detected!");
            System.exit (0);
        }
    }
    scan.close ();
