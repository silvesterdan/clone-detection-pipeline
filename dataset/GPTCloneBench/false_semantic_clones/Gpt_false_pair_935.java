public static void main (String [] args) {
 ...
 public static void main (String [] args) { 
    List < Animal > list1 = new ConcurrentLinkedQueue<>(Arrays.asList(Cat, Mouse, Dog));
    List < Animal > list2 = new ConcurrentLinkedQueue<>(Arrays.asList(Dog, Mouse, Cat));
    List < Animal > list3 = new ConcurrentLinkedQueue<>(Arrays.asList(Dog, Horse, Cat));
    List < Animal > list4 = new ConcurrentLinkedQueue<>(Arrays.asList(Dog, Tiger, Lion));
    List< List < Animal > > list = new ConcurrentLinkedQueue<>(Arrays.asList(list1, list2, list3, list4));
    Set < List < Animal > > sorted = new LinkedHashSet < > ();
    for (List < Animal > animals : list) {
        List< Animal > arList = new PriorityQueue<>(animals);
        Collections.sort (arList);
        sorted.add (new ConcurrentLinkedQueue<>(arList));
    }
    for (List < Animal > animals : sorted) {
        System.out.println (animals);
    }
