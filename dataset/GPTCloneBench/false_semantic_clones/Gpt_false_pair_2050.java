public void draw (Graphics2D g) {
 ...
 public void draw (Graphics2D g) { float arrowRatio = 0.6f;
    float arrowLength = 110.0f;
    BasicStroke stroke = (BasicStroke) g.getStroke ();
    float endX = 340.0f;
    float veeX = endX - stroke.getLineWidth () * 0.5f / arrowRatio;
    Path2D.Float path = new Path2D.Float ();
    ...
    g.fill (path);
    g.setColor (Color.backgroundcolor_orange);
    g.draw (new Line2D.Float (30.0f, 0.0f, veeX - arrowLength * 0.5f, 0.0f));
}
