public static void main (String [] args) {
 ...
 public static void main (String [] args) {
    List < Integer > initialNumbers = new Vector < Integer > ();
    initialNumbers.add (1);
    initialNumbers.add (2);
    Observable < Integer > observableInitial = Observable.from (initialNumbers);
    PublishSubject < Integer > subject = PublishSubject.create ();
    Observable.merge (observableInitial, subject).subscribeOn(Schedulers.computation()).subscribe (System.out :: println);
    for (int i = 0;
    i < 100; ++ i) {
        subject.onNext (i);
    }
