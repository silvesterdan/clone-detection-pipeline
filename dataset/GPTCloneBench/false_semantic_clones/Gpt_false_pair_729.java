private static void printPath (int [] [] mat, int i, int j) {
  ...
  private static void printPath (int [] [] mat, int i, int j) {
    ...
    int rightValue = (i < mat.length - 1) && (mat[i][j] < mat[i+1][j]) ? mat[i+1][j] : mat [i][j];
    int leftValue = (i > 0) && (mat[i][j] < mat[i-1][j]) ? mat[i-1][j] : mat [i][j];
    int belowValue = (j < mat[0].length - 1) && (mat[i][j] < mat[i][j+1]) ? mat[i][j+1] : mat [i][j];
    int aboveValue = (j > 0) && (mat[i][j] < mat[i][j-1]) ? mat[i][j-1] : mat [i][j];
