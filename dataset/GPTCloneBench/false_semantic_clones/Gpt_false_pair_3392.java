...
public static void main (String args []) {
    ...
    for (i = large.length; i < array.length; i ++) {
        if (smallest < array [i]) {
            large [smallestIndex] = array [i];
            int smallestCopy = large [0];
            int smallestIndexCopy = 0;
            for (int k = 1;
            k < large.length; ++ k) {
                if (smallestCopy > large [k]) {
                    smallestCopy = large [k];
                    smallestIndexCopy = k;
                }
            }
            smallest = smallestCopy;
            smallestIndex = smallestIndexCopy;
        }
    }
    for (int j = 0;
    j < large.length; j ++) {
    ...
    System.out.println ();
    System.out.println ("Largest is: " + getGreatest (large));
}
