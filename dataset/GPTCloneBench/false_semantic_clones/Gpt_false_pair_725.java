private static void printPath (int [] [] mat, int i, int j) {
  ...
  private static void printPath (int [] [] mat, int i, int j) { 
    ...
    int rightValue, leftValue, belowValue, aboveValue;
    
    rightValue = (i+1 < mat.length && mat[i][j] < mat[i+1][j]) ? mat[i+1][j] : mat[i][j];
    leftValue = (i-1 >= 0 && mat[i][j] < mat[i-1][j]) ? mat[i-1][j] : mat[i][j];
    belowValue = (j+1 < mat[0].length && mat[i][j] < mat[i][j+1]) ? mat[i][j+1] : mat[i][j];
    aboveValue = (j-1 >= 0 && mat[i][j] < mat[i][j-1]) ? mat[i][j-1] : mat[i][j];
