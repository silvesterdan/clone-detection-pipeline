...
public static void countString (String str, char value) {
    ...
    for (String s : arr) {
        tempString.append (s);
        for (char ch : s.toCharArray ()) {
            System.out.println ("Number of Occurrence of " + ch + " is:" +tempString.toString ().chars ().filter (i -> i == ch).count ());
        }
    }
    if (! (Character.toString (value).isEmpty ())) {
        ...
        for (String s : arr) {
            ...
            for (char ch : s.toCharArray ()) {
                if (ch == value) {
                    System.out.println ("Number of Occurrence of " + ch + " is:" +tempString2.toString ().chars ().filter (i -> i == ch).count ());
                }
            }
        }
    }
