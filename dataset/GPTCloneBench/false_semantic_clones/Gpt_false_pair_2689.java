public Serializable generate (SessionImplementor session, Object object) throws HibernateException {
 ...
 public Serializable generate(SessionImplementor session, Object object) throws HibernateException {
    ...
    try {
        ...
        statement = connection.createStatement();
        try {
            resultSet = statement.executeQuery("SELECT next_val FROM my_seq");
        }
        catch (Exception e) {
            System.out.println ("In catch, cause : Table is not available.");
            statement.execute ("CREATE sequence my_seq");
            statement.executeUpdate ("ALTER SEQUENCE my_seq INCREMENT BY 1 START WITH 0");
            resultSet = statement.executeQuery ("SELECT next_val FROM  my_seq");
        }
        if (resultSet.next()) {
            int nextValue = resultSet.getInt (1);
