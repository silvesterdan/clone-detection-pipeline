public Serializable generate (SessionImplementor session, Object object) throws HibernateException {
 ...
 public Serializable generate(SessionImplementor session, Object object) throws HibernateException {
    ...
    try {
        ...
        statement = connection.createStatement();
        try {
            resultSet = statement.executeQuery("SELECT setval ('seq_id', coalesce(max(id)+1, 1), false) FROM my_data");
        }
        catch (Exception e) {
            System.out.println ("In catch, cause : Sequence is not available.");
            statement.execute ("CREATE sequence seq_id");
            statement.executeUpdate ("ALTER SEQUENCE seq_id INCREMENT BY 1 START WITH 0");
            resultSet = statement.executeQuery ("SELECT setval ('seq_id', coalesce(max(id)+1, 1), false) FROM my_data");
        }
        if (resultSet.next()) {
            int nextValue = resultSet.getInt (1);
