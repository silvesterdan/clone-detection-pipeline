public void invoke (Request request, Response response) throws IOException, ServletException {
	...
	public void invoke(Request request, Response response) throws IOException, ServletException {
        ...
        try {
            ...
            log.error(getInfo() + ": Unable to satisfy the replication request.", e);
        }
        String context = request.getContext().getName();
        String task = request.getPathInfo();
        if (task == null) {
        ...
        if (session != null) {
            ...
            if (session instanceof ReplicatedSession) {
                ((ReplicatedSession) session).setIsDirty(true);
                if (log.isDebugEnabled()) {
                    log.debug(getInfo() + ": [session=" + session.getId() + ", context=" + context + ", request=" + task + "] set to DIRTY");
                }
            } else {
                List cycledNames = new ArrayList<>();
                HttpSession deltaSession = (HttpSession) session;
                for (Enumeration<String> names = deltaSession.getAttributeNames();
                names.hasMoreElements(); ) {
                    String name = names.nextElement();
                ...
                if (log.isDebugEnabled()) {
                    log.debug(getInfo() + ": [session=" + session.getId() + ", context=" + context + ", request=" + task + "] recycle atrributes=" + cycledNames + "");
                }
            }
        } else {
            String id = request.getRequestedSessionId();
            log.warn(getInfo() + ": [session=" + id + ", context=" + context + ", request=" + task + "] Session is not available, unable to send session over to the cluster.");
        }
}
