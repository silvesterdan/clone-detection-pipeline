 ...
 public static void main (String args []) {
    ...
    try {
        ...
        while ((line = br.readLine ()) != null) {
            ...
            while (st.hasMoreTokens ()) {
                String nextElement = (String) st.nextElement ();
                if (ihm.size () > 0 && ihm.containsKey (nextElement)) {
                    int val = 0;
                    if (ihm.get (nextElement) != null) {
                        val = (Integer) ihm.get (nextElement);
                        val = val + 1;
                    }
                    ihm.put (nextElement, val);
                } else {
                    ihm.put (nextElement, 1);
                }
            }
        }
        for (Map.Entry < String, Integer > entry : ihm.entrySet ()) {
            System.out.println (entry.getKey () + " : " + entry.getValue ());
        }
    } catch (FileNotFoundException e) {
        e.printStackTrace ();
